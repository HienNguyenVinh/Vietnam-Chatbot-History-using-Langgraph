...
web_search_tool = TavilySearch(tavily_api_key=TAVILY_API_KEY, 
                                ...
                                include_domains=["https://baochinhphu.vn/", "https://www.tapchicongsan.org.vn/", "https://vanhoavaphattrien.vn/", "https://luocsutocviet.com/", "https://vov2.vov.vn/", "https://nhandan.vn/", "https://laodong.vn/"],
                                country="vietnam",)
...
async def generate_query_or_respond(state: AgentState):
    ...
    response = await llm_model.bind_tools([rag]).ainvoke(messages)  
...
async def grade_documents(state: AgentState) -> Literal["generate_answer", "web_search"]:
